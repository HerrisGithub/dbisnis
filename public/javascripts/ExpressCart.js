$(document).ready(function() {	
	...
	$("input[class='published_state']").change(function() {
		$.ajax({
			method: "POST",
			url: "/penjual/product/published_state",
    ...
    $("#order_status_update").click(function() {
        $.ajax({
			method: "POST",
			url: "/penjual/order/statusupdate",
    ...
    $(".add-to-cart").click(function() {
        $.ajax({
			method: "POST",
			url: "/penjual/product/addtocart",
			data: { product_id: $(this).attr("data-id") ,product_by:$('.product_by').attr("data-id")}
		})
		.success(function(msg) {
    ...
    $(".cart-delete-button").click(function() {
        $.ajax({
			method: "POST",
			url: "/penjual/product/removefromcart",
    ...
    $(".cart-update-button").click(function() {
        $.ajax({
			method: "POST",
			url: "/penjual/product/updatecart",
    ...
    $('.thumbnail-image').on('click', function(){
		...
		$('#product-title-image-href').attr('href','javascript:ImageZoom()');
    });
    ...
    $('.set-as-main-image').on('click', function(){
        $.ajax({
			method: "POST",
			url: "/penjual/product/setasmainimage",
			data: { product_id: $("#frm_id").val(), product_image: $(this).attr("data-id") }
		})
		.success(function(msg) {
            show_notification(msg.message,"success", true); 
        })
    ...
    $('.btn-delete-image').on('click', function(){
        $.ajax({
			method: "POST",
			url: "/penjual/product/deleteimage",
			data: { product_id: $("#frm_id").val(), product_image: $(this).attr("data-id") }
		})
		.success(function(msg) {
            show_notification(msg.message,"success", true); 
	...
	$("#validate_permalink").click(function() {
		...
		var data={
			"permalink" : $("#frm_product_permalink").val(),
			"doc_id": $("#frm_product_id").val()
		};
		if($("#frm_product_permalink").val() != ""){
			$.ajax({
				method: "POST",
				url: "/penjual/api/validate_permalink",
				data:data
			})
			.success(function(msg) {
				show_notification(msg,"success");
	...
	$("#btn_product_filter").click(function() {
        if($("#product_filter").val() != ""){
            window.location.href = "/penjual/products/filter/" + $("#product_filter").val();
        }else{
	...
	$("#btn_order_filter").click(function() {
        if($("#order_filter").val() != ""){
		  window.location.href = "/penjual/orders/filter/" + $("#order_filter").val();
         }else{
            show_notification("Please enter a keyword to filter","danger");
        }
	});
	...
	$("#btn_product_reset").click(function() {
		window.location.href = "/penjual/products";
	});
	...
	$(document).scroll(function (e) {  
		...
		if(top===0){
			if(!detectmob()){
				...
				$('#navLogo').css({backgroundImage:''});	
				// $('#content').css({paddingTop:'0'});
				// $('#rightFixed').css({marginTop:'100px'});
					// $('#rightFixed').css({height:'74.9%'});
			...
			$('#navLogo').css({backgroundImage:"url('/dirbis.png')"});	
			// $('#content').css({paddingTop:'77px'});
			$('.rs').css({top:'0%'});
		}	
