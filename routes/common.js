...
exports.check_login_pembeli = function(req, res, next){
    if(req.session.user||req.session.passport){
        if(req.session.user){
            ...
            requestify.get('http://127.0.0.1:5000/me/facebook')
            .then(function(response) {
                var resp = response.getBody();
                var username=resp.username;
                model.pembeli.where('username',username)
                .fetch().then(function (usr) {  
                    usr=JSON.parse(JSON.stringify(usr));
                    if(typeof usr.username!=='undefined'){
                        next();
                        return;
                    }else{
                        res.send(401);
...
exports.update_total_cart_amount = function(req, res){
    ...
    async.each(req.session.cart, function(cart_product, callback) {
        req.session.total_cart_amount = parseFloat(req.session.total_cart_amount) + parseFloat(cart_product.item_price);
        callback();
    });
...
exports.get_images = function (dir, req, res, callback){
    ...
    model.products.where({id: dir}).fetch().then(function (product) {
        ...
        glob("public/uploads/product_image/" + dir +"/**", {nosort: true}, function (er, files) {
            // sort array
            files.sort();
